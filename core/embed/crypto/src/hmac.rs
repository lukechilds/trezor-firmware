...
impl HmacSha256Ctx {
    ...
    /// # Safety
    ///
    /// In certain scenarios, like if the context lives on the stack, Rust may
    /// silently make copies of the context, leaving possibly sensitive state
    /// (which will, after this operation, include data derived from the key) in
    /// memory even after disposing of the context.
    ///
    /// To ensure secure handling, this method should be called through an
    /// indirection on a stable memory location, such as through a `Pin`.
