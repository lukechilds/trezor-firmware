...
impl Hash for TrezorCryptoSha256 {
    ...
    fn input(&mut self, data: &[u8]) {
        // SAFETY: Usage is sound. The unsafe marker on update_raw does not
        // break soundness.
        //
        // Otherwise not actually safe! This struct breaks the assumption that
        // the hasher's inner state cannot be copied around. We have to live
        // with that here, because we can't put the storage on the heap, so it
        // needs to be owned, which prevents us from creating a safe interface.
        unsafe { self.0.update_raw(data) };
    }

    fn result(&mut self) -> Self::Output {
        // SAFETY: sound but unsafe, see `input()` above for details
...
impl Default for TrezorCryptoSha256 {
    fn default() -> Self {
        let mut memory = Memory::<sha256::Sha256Ctx>::default();
        memory.init_raw();
        Self(memory)
    }
}
