...
fn sha3_init_raw(ctx: &mut ffi::SHA3_CTX, bit_size: u32) -> Result<(), Error> {
    // SAFETY: ffi
    if unsafe { ffi::keccak_Init(ctx, bit_size) } {
        Ok(())
    } else {
